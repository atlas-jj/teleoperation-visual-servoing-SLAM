...
*/
transformation2D::transformation2D(cv::Point _t, double _scale, double _rotation)
{
  t=_t;
  scale=_scale;
  rotation=_rotation;
...
Point2d transformation2D::doTransformation(cv::Point2d p1)
{
   ...
   cv::Mat_<double> transformation(2,3);
...
vector< vector <Point2d> > transformation2D::doTransformation(cv::Point2d _transPoint, vector < vector < cv::Point> > _inputPoints, double _scale, double _rotation)
{
  scale=_scale;
  rotation=_rotation;
  vector< vector < cv::Point2d> > outputPoints;
  cv::Point relativeTransPoint(0,0);
  ...
  {
    vector<cv::Point2d> strokePoints;
    size_t pointNum=_inputPoints[m].size();
    for(int n=0;n<pointNum;n++)
    {
      cv::Point2d tfPoint=doTransformation(_inputPoints[m][n]);
